...
export const buildValidationSchema = (config) => {
    ...
    config.forEach((field) => {
        ...
        if (field.isFile || field.isImage) {
            ...
            if (field.isImage) {
                fileSchema = fileSchema.test('fileType', 'Only image files are allowed (jpg, png)', (value) => value instanceof File && ['image/jpeg', 'image/png'].includes(value.type));
            }
            ...
            if (field.type === 'email')
                base = base.email('Invalid email format');
            if (field.type === 'password')
                base = base.min(6, 'Password must be at least 6 characters');
            if (field.type === 'number')
                base = base.matches(/^\d+$/, 'Only numbers are allowed');
